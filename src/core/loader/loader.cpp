...
std::optional<std::pair<Loader::AppLoader::CompressFileInfo, size_t>> GetCompressFileInfo(
    ...
    size_t frame_size{};
    // CIA files must keep using the install-style compression path below, not
    // the direct-boot loader (which only sees the main content).
    std::string extension;
    Common::SplitPath(filepath, nullptr, nullptr, &extension);
    const bool is_cia = GuessFromExtension(extension) == FileType::CIA;
    std::unique_ptr<AppLoader> loader;
    if (!is_cia) {
