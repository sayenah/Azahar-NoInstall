...
#elif defined(ANDROID) && !defined(HAVE_LIBRETRO_VFS)
    if (AndroidStorage::CanUseRawFS()) {
        m_file = FOPEN(AndroidStorage::TranslateFilePath(open_path).c_str(), openmode.c_str());
    } else {
        ...
        m_fd = AndroidStorage::OpenContentUri(open_path, android_open_mode);
        if (m_fd != -1) {
            int error_num = 0;
...
std::size_t IOFile::ReadImpl(void* data, std::size_t length, std::size_t data_size) {
    ...
    if (is_virtual && data_size != 0) {
        // Clamp reads so they cannot escape the virtual view.
        const u64 pos = TellImpl();
        const u64 remaining = pos < virt_size ? virt_size - pos : 0;
        length = std::min<std::size_t>(length, static_cast<std::size_t>(remaining / data_size));
        if (length == 0) {
            return 0;
        }
    }
